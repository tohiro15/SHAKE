...
namespace Steamworks
{
	public static class CallbackDispatcher
	{
		...
		internal static void Register(Callback cb)
		{
			...
			lock (m_sync)
			{
				if (!dictionary.TryGetValue(callbackIdentity, out List<Callback> value))
				{
		...
		internal static void Register(SteamAPICall_t asyncCall, CallResult cr)
		{
			lock (m_sync)
			{
				if (!m_registeredCallResults.TryGetValue((ulong)asyncCall, out List<CallResult> value))
				{
			...
			while (NativeMethods.SteamAPI_ManualDispatch_GetNextCallback(hSteamPipe, m_pCallbackMsg))
			{
				...
				try
				{
					...
					if (callbackMsg_t.m_iCallback == 703)
					{
						...
						if (NativeMethods.SteamAPI_ManualDispatch_GetAPICallResult(hSteamPipe, steamAPICallCompleted_t.m_hAsyncCall, intPtr, (int)steamAPICallCompleted_t.m_cubParam, steamAPICallCompleted_t.m_iCallback, out bool pbFailed))
						{
							lock (m_sync)
							{
								if (m_registeredCallResults.TryGetValue((ulong)steamAPICallCompleted_t.m_hAsyncCall, out List<CallResult> value))
								{
					...
					else if (dictionary.TryGetValue(callbackMsg_t.m_iCallback, out value2))
					{
						List<Callback> list;
						lock (m_sync)
						{
